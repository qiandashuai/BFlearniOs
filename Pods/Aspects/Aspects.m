...
typedef struct _AspectBlock {
	__unused Class isa; //旨在告诉编译器“如果我没有在后面使用到这个变量也别警告我”。
...
// 一个 aspect 执行环境，主要是 NSInvacation 信息。
// aspectInfo 扮演了一个提供 Hook 信息的角色
...
// 一个 aspect 的具体内容
// aspectIdentifier 实际上是添加切面的 Block 的第一个参数，
@interface AspectIdentifier : NSObject
// 实力化。 SEL, object, 执行的东西
+ (instancetype)identifierWithSelector:(SEL)selector object:(id)object options:(AspectOptions)options block:(id)block error:(NSError **)error;
- (BOOL)invokeWithInfo:(id<AspectInfo>)info;
@property (nonatomic, assign) SEL selector;
@property (nonatomic, strong) id block;
@property (nonatomic, strong) NSMethodSignature *blockSignature; //描述
...
static id aspect_add(id self, SEL selector, AspectOptions options, id block, NSError **error) {
    ...
    // this block is a lock
    aspect_performLocked(^{
        
        // 如果允许被 hook
        if (aspect_isSelectorAllowedAndTrack(self, selector, options, error)) {
            
            //_aspectkey: contanier
...
// 返回一个 block 的 NSMethodSignature.
...
static Class aspect_hookClass(NSObject *self, NSError **error) {
    ...
    // object_getClass [obj class]: 如果是实例对象，则返回 isa 指针，类对象，object_class 返回 isa, class 返回 本身
...
// 将该类的 forwardInvocation: 的实现替换成这个。
static void __ASPECTS_ARE_BEING_CALLED__(__unsafe_unretained NSObject *self, SEL selector, NSInvocation *invocation) {
    ...
    // 无差别获取 可能是实例Contianer 亦或者是类 Container
...
static AspectsContainer *aspect_getContainerForObject(NSObject *self, SEL selector) {
    ...
    if (!aspectContainer) {
        ...
        // aspects_selectorname: aspectConatiner
        // 被这个 放在了 self 这个类实例里面
    }
